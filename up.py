...
bot_token = '7226615338:AAF10ETNZnOkYYr3JzjG0vKPbGdf-vis6x8' # Replace with your bot token
admin_id = '6155412817'  # Replace with your actual admin user ID (as a string)
GROUP_ID = '-1002643293106'  # Allowed group ID where commands work AND group cooldown applies (as a string)
...
REQUIRED_CHANNELS = ['@AashuZod', '@AASHUFREEMODZ', '@AASHUZODFREEMODZ'] # Example: ['@mychannel1', '-1001234567890']
        ...
        if sender_user_id == admin_id or sender_user_id in approved_private_users:
            ...
            if REQUIRED_CHANNELS:
                 if check_channel_membership(sender_user_id):
                      ...
                      channel_links = "\n".join([f"➡️ [{ch.replace('@AASHUFREEMODZ', '')}](https://t.me/{ch.replace('@AashuZod', '')})" for ch in REQUIRED_CHANNELS])
                      welcome_message += (
...
def show_help(message):
    ...
    # FIX: Wrap the @ mention in backticks
    help_lines.append(f"❤️ Credit: @AashuZodYt")
